  ...
  async ({ name }) => {
    return {
      content: [
        {
          type: "text",
          text: `Hello, ${name}! Welcome to the DunderMifflin MCP server.`
        }
      ]
    };
  }
...
server.tool(
  ...
  async ({ policyId, callerRole }) => {
    ...
    if (!policy) {
      return {
        content: [
          { type: "text", text: `No policy found with id "${policyId}". Use list_policies to see valid IDs.` }
        ]
      };
    }

    const callerRank = roleRank[callerRole];
    const visibleSections = policy.sections.filter(
      (section) => roleRank[section.minRole] <= callerRank
    );
